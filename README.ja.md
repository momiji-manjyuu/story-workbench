...
英語版: [README.md](/Users/user/story-workbench/README.md)
...
5. 本文を書く、または改稿する
6. 結果を `data/state/` や `data/plot/` に反映する
7. `python3 tools/story.py checkpoint ...` で今回の確定事項と保留事項を記録する
   `checkpoint` はデフォルトで validation / semantic audit のエラーが残っていると失敗します。意図的に壊れた状態も記録したい時だけ `--allow-findings` を使います。
8. `validate` と `audit` を再実行する
